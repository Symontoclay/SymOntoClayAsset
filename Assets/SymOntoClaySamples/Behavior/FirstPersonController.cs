...
{
    [AddComponentMenu("SymOntoClay Samples/FirstPersonController")]
        ...
        private Vector2 GetInput()
        {
            ...
            if(input == Vector2.zero)
            {
                if(_movementState != MovementState.Stops)
                {
                    ...
                    AddStopFact(Logger);
                    StopRepeatingStepsSoundInMainThread(Logger);
                }
            }
            else
            {
                if(movementSettings.Running)
                {
                    if(_movementState != MovementState.Runs)
                    {
                        ...
                        StartRepeatingRunningStepsSoundInMainThread(Logger);
                    }
                }
                else
                {
                    if(_movementState != MovementState.Walks)
                    {
                        ...
                        StartRepeatingWalkingStepsSoundInMainThread(Logger);
                    }
                }
            }
